...
impl fmt::Display for TiafArgs {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "TiafClient {{ ip: {}, port: {}, log_level: {} }}",
            self.host, self.port, self.log_level
        )
    }
}
...
fn main() {
    let command_structure = Command::new("tiaf")
        ...
        .subcommand(Command::new("statistics").short_flag('S'))
        .subcommand(
            Command::new("chain")
                .short_flag('C')
                .subcommand(
                    Command::new("full-read")
                        .short_flag('F')
                        .about("read the full chain"),
                )
                .subcommand(
                    Command::new("partial-read")
                        .short_flag('P')
    ...
    let _logger = woody::new(woody::Level::from_u8(&global_args.log_level).unwrap());

    println!("args: {global_args}");

    if let Some(sub_m) = matches.subcommand_matches("chain") {
        if let Some(_sub_m) = sub_m.subcommand_matches("full-read") {
        ...
        if let Some(_sub_m) = sub_m.subcommand_matches("partial-read") {
    ...
    if let Some(_sub_m) = matches.subcommand_matches("statistics") {
    ...
    if let Some(sub_m) = matches.subcommand_matches("query") {
        ...
        let query = inline
            .map(|s| s.to_string())
            .or_else(|| match fs::read_to_string(file.unwrap()) {
